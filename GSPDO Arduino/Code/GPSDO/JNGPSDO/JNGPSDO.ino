...
#define USE_PHASE_DETECTOR  false       // Set to false if the XTAL is too unstable to phase lock
...
#ifndef DAC  // Not used for DAC implementation
 SI5351aStart();  
...
#ifdef DEBUG
  Serial.println(F("DBG - DAC begin"));
#endif  
  //dac.begin(0x60);  // NR
  dac.begin();
#endif
...
#ifdef DEBUG
  Serial.println(F("DBG - Setting DAC"));
#endif  
...
//   dac.setVoltage(dac_out, false); //NR
    dac.setValue(dac_out); //NR
...
#ifdef DEBUG
	if (FirstLoopPass == true) {
	 Serial.println(F("DBG - Loop - First Pass"));
	 FirstLoopPass = false;
	}
#endif  
...
// after an hour or so
{
  ...
  if (avgfhun >= 10000000.01) {
    ...
    dac.setValue(adjusted_DAC_output); // min=0 max=4096
  } 

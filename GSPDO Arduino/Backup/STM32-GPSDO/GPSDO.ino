...
// 2021-11-06 NR Commented out the following
...
// NR #define VctlPWMOutputPin PB9          // digital output pin used to output a PWM value, TIM4 ch4
...
// NR #define blueledpin  PC13    // Blue onboard LED is on PC13 on STM32F411CEU6 Black Pill
// Yellow extra LED is off, on or blinking to indicate some GPSDO status
// NR#define yellowledpin PB8   // Yellow LED on PB8
...
//volatile uint64_t circbuf_tth64[10001]; // 10,000 + 1 seconds circular buffer // 2021-11-06 NR
...
// NR  analogWrite(VctlPWMOutputPin, adjusted_PWM_output);
...
{
// NR  adjusted_DAC_output = adjusted_DAC_output + 10;
// NR  dac.setVoltage(adjusted_DAC_output, false);
...
{
// NR  adjusted_DAC_output = adjusted_DAC_output + 1;
// NR  dac.setVoltage(adjusted_DAC_output, false);
...
// NR  digitalWrite(blueledpin, !digitalRead(blueledpin));
  ...
  if (captureflag) {
// NR    lsfcount = TIM2->CCR3; // read TIM2->CCR3
  ...
  switch (yellow_led_state)
  {
    case 0:
      // turn off led
// NR      digitalWrite(yellowledpin, LOW);
      break;
    case 1:
      // turn on led
// NR      digitalWrite(yellowledpin, HIGH);
      break;
    case 2:
      // blink led
// NR      digitalWrite(yellowledpin, !digitalRead(yellowledpin));
      break;
    default:
...
// NR  circbuf_tth64[cbitth_newest]=fcount64;
  cbitth_newest++;
...
// NR    if (cbitth_newest == 0) latfcount64 = circbuf_tth64[10000];
// NR    else latfcount64 = circbuf_tth64[cbitth_newest-1];
// NR    oldfcount64 = circbuf_tth64[cbitth_newest];
...
void setup()
{
  ...
  // Setup 2Hz Timer
// NR  HardwareTimer *tim2Hz = new HardwareTimer(TIM9);
  
  // configure blueledpin in output mode
// NR  pinMode(blueledpin, OUTPUT);

  // configure yellow_led_pin in output mode
// NR  pinMode(yellowledpin, OUTPUT);    
  
// NR  tim2Hz->setOverflow(2, HERTZ_FORMAT); // 2 Hz
// NR  tim2Hz->attachInterrupt(Timer_ISR_2Hz);
// NR  tim2Hz->resume();

  // Setup serial interfaces
  Serial.begin(115200); // USB serial
  Serial0.begin(9600);  // Hardware serial 1 to GPS module
  #ifdef GPSDO_BLUETOOTH
...
// NR  adjusted_DAC_output = default_DAC_output; // initial DAC value
// NR  dac.begin(0x60);
  // Output Vctl to DAC, but do not write to DAC EEPROM 
// NR  dac.setVoltage(adjusted_DAC_output, false); // min=0 max=4096 so 2048 should be 1/2 Vdd = approx. 1.65V
...
// NR  analogWrite(VctlPWMOutputPin, 127);      // configures PB9 as PWM output pin at default frequency and resolution
...
// NR  analogWrite(VctlPWMOutputPin, adjusted_PWM_output);    // 32767 for 16 bits -> 50% duty cycle so a square wave
...
// NR  TIM2->ARR = 0xffffffff; // count to 2^32, then wraparound (approximately every 429 seconds)
...
// NR  TIM2->SMCR |= TIM_SMCR_ECE; // 0x4000
...
// GPSDO tunnel mode operation
{
  ...
  {
// NR    if (Serial1.available() > 0)
    {
// NR      GPSchar = Serial1.read();
      Serial.write(GPSchar);  // echo NMEA stream to USB serial
    }
    if (Serial.available() > 0)
    {
      PCchar = Serial.read();
// NR      Serial1.write(PCchar);  // echo PC stream to GPS serial
    }
  }
...
// NR  analogWrite(VctlPWMOutputPin, 30720);
  delay(15000);
...
// NR  analogWrite(VctlPWMOutputPin, 51200);
  delay(15000);
...
// NR      adjusted_DAC_output--;
    }
// NR    dac.setVoltage(adjusted_DAC_output, false); // min=0 max=4096
  } 
...
void printGPSDOstats(Stream &Serialx) 
{
  ...
  Serialx.print(Vctl);
  Serialx.print("  DAC: ");
// NR  Serialx.println(adjusted_DAC_output);
